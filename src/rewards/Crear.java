...
public class Crear {
    ...
    public static void addAlmacen(String botin) {
        try {
            ...
            rootElement.appendChild(createElement(document, "origin", "Álex Varela"));
            ...
            // Crear el archivo XML en la carpeta "recompensas" con pretty-printing
            File file = new File(folder, "almacen_" + botin.toLowerCase() + ".xml");
            TransformerFactory transformerFactory = TransformerFactory.newInstance();
            Transformer transformer = transformerFactory.newTransformer();
            transformer.setOutputProperty(OutputKeys.INDENT, "yes"); // Activar pretty-printing
            transformer.setOutputProperty("{http://xml.apache.org/xslt}indent-amount", "4"); // Número de espacios por
                                                                                             // nivel
    ...
    public static int obtenerValor(String rutaArchivo) {
        try {
            ...
            EscritorHelper.getEscritorHelper("").addError("Error al crear el elemento xml");
        }
        return 1;
    }
}